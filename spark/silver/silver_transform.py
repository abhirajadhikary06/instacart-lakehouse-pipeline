...
slv_order_products__prior_df = spark.read.format("delta").load(
    f"{SILVER_BASE}/order_products__prior"
)

# Join-1 products and aisles and departments
products_join_df = slv_products_df.join(
    broadcast(slv_aisles_df.select("aisle_id", "aisle")), "aisle_id"
).join(broadcast(slv_departments_df.select("department_id", "department")), "department_id")

products_join_df.write.format("delta").mode("overwrite").option("overwriteSchema", "true").save(
    f"{SILVER_BASE}/slv_products_enriched"
)
...
order_products_prior_join_df = slv_order_products__prior_df.join(
    broadcast(
        slv_products_enriched_df.select(
...
order_products_prior_join_df.write.format("delta").mode("overwrite").option(
    "overwriteSchema", "true"
).save(f"{SILVER_BASE}/slv_order_products_prior_enriched")
order_products_prior_enriched_df = spark.read.format("delta").load(
    f"{SILVER_BASE}/slv_order_products_prior_enriched"
)

# Join-3 enriched order_products and orders
order_products_full_df = order_products_prior_enriched_df.join(
    slv_orders_df.select(
        "order_id",
        "user_id",
        "order_number",
        "order_dow",
...
order_products_full_df.write.format("delta").mode("overwrite").option(
    "overwriteSchema", "true"
).save(f"{SILVER_BASE}/slv_order_products_full")
