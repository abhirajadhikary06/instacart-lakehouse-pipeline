...
orders_df = (
    orders_df.withColumn("is_first_order", when(col("order_number") == 1, True).otherwise(False))
    .withColumn(
        "is_weekend", when((col("order_dow") == 0) | (col("order_dow") == 1), True).otherwise(False)
    )
...
def main():
    ...
    write_silver(
        deduplicate(order_products__prior_df, ["order_id", "product_id"]), "order_products__prior"
    )
