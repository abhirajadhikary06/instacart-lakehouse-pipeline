...
with logo_col2:
    if LOGO_PATH.exists():
        ...
        st.markdown(
            f"<h1 style='text-align:center; color:{INSTACART_GREEN}'>🛒 Instacart Lakehouse</h1>",
            unsafe_allow_html=True,
        )
...
@st.cache_resource
def get_engine():
    return create_engine("postgresql+psycopg2://dwh_user:dwh_password@localhost:5434/instacart_dwh")
...
if page == "Product Popularity":
    ...
    df = query(
        "SELECT * FROM dev_marts.mart_product_popularity ORDER BY reorder_rate DESC LIMIT 50"
    )

    dept = st.selectbox(
        "Filter by department", ["All"] + sorted(df["department"].unique().tolist())
    )
    if dept != "All":
    ...
    with col1:
        fig = px.bar(
            ...
            x="department",
            y="total_orders",
            color="reorder_performance",
            title="Total Orders by Department",
            color_discrete_map={
                "High": INSTACART_GREEN,
                "Medium": INSTACART_ORANGE,
                "Low": INSTACART_DARK_GREEN,
            },
        )
        fig.update_layout(xaxis_tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig2 = px.bar(
        df.groupby("traffic_category")["total_orders"].sum().reset_index(),
        x="traffic_category",
        y="total_orders",
        color="traffic_category",
        title="Orders by Traffic Category",
        color_discrete_map={
            "Peak": INSTACART_ORANGE,
            "High": INSTACART_GREEN,
            "Moderate": INSTACART_DARK_GREEN,
            "Low": "rgba(10,173,5,0.4)",
        },
    )
    ...
    fig = px.pie(
        ...
        color="segment",
        color_discrete_map={
            "High Value": INSTACART_GREEN,
            "Regular": INSTACART_ORANGE,
            "Occasional": INSTACART_DARK_GREEN,
        },
    )
    st.plotly_chart(fig, use_container_width=True)
